...
function Playing({ operationSettings, time, switchToResults }: { operationSettings: Record<string, Record<string, any>>, time: number, switchToResults: Function }) {
    ...
    const newCalculation = useCallback(() => {
        ...
        if (operationSettings.addition.enabled) possibleOperations.push(["addition", "+"]);
        if (operationSettings.subtraction.enabled) possibleOperations.push(["subtraction", "-"]);
        if (operationSettings.multiplication.enabled) possibleOperations.push(["multiplication", "×"]);
        if (operationSettings.division.enabled) possibleOperations.push(["division", "÷"]);

        function getRandomOperation() {
            return possibleOperations[Math.floor(Math.random() * possibleOperations.length)]
        }
        ...
        function calculateAnswer(firstNumber: number, secondNumber: number, operationSymbol: string) {
        ...
        if (operation === "division") {
            ...
            const secondNumber = getRandomNumber(settings.secondMin, settings.secondMax)
    ...
    return (
        <div className="flex items-center justify-center w-full h-screen">
            ...
            <span className="absolute top-2 right-2">Score: <strong className="font-medium">{score}</strong></span>

            <div className='p-8 flex w-full items-center justify-center bg-gray-200 gap-4'>
                <h1 className="text-3xl"><span className="font-semibold">{calculationQuestion}</span> =</h1>
                <input
                    type="number"
                    step={1}
                    autoFocus
